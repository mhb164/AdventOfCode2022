...
[PuzzleDayNumber(12)]
public class Day12SecondTrySolution : PuzzleSolution
{
    ...
    class MapItem
    {
        ...
        internal bool CanClimpUp(Climbing climbing, MapItem another)
        {
            if (another == null) return false;
            if (climbing.Passed.Contains(another)) return false;
            if (climbing.Visited.Contains(another)) return false;
            if (Height >= another.Height - 1) return true;
            return false;
        }

        internal bool CanClimpUp(MapItem another)
        {
            if (another == null) return false;
            if (Height >= another.Height - 1) return true;
            return false;
    ...
    class Climbing
    {
        public List<MapItem> Passed = new List<MapItem>();
        public List<MapItem> Visited = new List<MapItem>();
        public MapItem Item;
        public Climbing(MapItem item)
        {
            Passed = new List<MapItem>
            {
                item
            };
            Visited = new List<MapItem>
            {
                item
            };
            Item = item;
        }
        public Climbing(Climbing another, MapItem item, List<MapItem> visited)
        {
            Passed = another.Passed.ToList();
            Passed.Add(item);

            Visited = another.Visited.ToList();
            Visited.AddRange(visited);
            Item = item;
        }

        public Climbing CreateAnother(MapItem item, List<MapItem> visited) => new Climbing(this, item, visited);
        public override string ToString() => $"({Passed.Count}) {string.Join(", ", Passed)}";
    }
